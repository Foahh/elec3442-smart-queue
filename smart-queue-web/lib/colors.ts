...
export const BUSYNESS_COLORS: Record<BusynessLevel | "stale", { hex: string; label: string }> = {
...
export function busynessColor(level: string, stale: boolean): string {
  ...
  return BUSYNESS_COLORS[level as BusynessLevel]?.hex ?? BUSYNESS_COLORS.stale.hex
}
